...
app.get('/', (_req: Request, res: Response) => {
	...
	const apiInfo = {
			...
			availableEndpoints: [
				{
					path: '/',
					method: 'GET',
					description: 'API documentation and welcome message (this endpoint)',
					example: `${baseUrl}/`
				},
				{
					path: '/health',
				...
				},
				{
					path: '/foods',
					method: 'GET',
					description: 'Search for food items in the USDA FoodData Central database',
					queryParameters: {
						type: {
							required: true,
							type: 'string',
					...
					example: `${baseUrl}/foods?type=apple&limit=5`
				}
			],
			rateLimiting: {
				windowMs: '15 minutes',
		...
		usdaDisclaimer: {
			important: 'USDA Data Disclaimer - Required Notice',
			message: 'This API provides access to data from the USDA FoodData Central database. The U.S. Department of Agriculture requires the following disclaimer for all uses of their data:',
